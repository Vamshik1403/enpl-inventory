...
const UpdateProductModal: React.FC<UpdateProductModalProps> = ({
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      if (productId) {
        try {
          const response = await axios.get(`http://http://192.168.29.167:8000/products/${productId}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`http://http://192.168.29.167:8000/products/${productId}`, updatedProduct);
