...
const VendorTable: React.FC = () => {
  ...
  const fetchVendors = async () => {
    const response = await axios.get("http://192.168.0.102:8000/vendors");
  ...
  const handleDelete = async (id?: number) => {
    ...
    try {
      await axios.delete(`http://192.168.0.102:8000/vendors/${id}`);
 ...
 const handleCreate = async () => {
  ...
  try {
    ...
    if (formData.id) {
      await axios.put(`http://192.168.0.102:8000/vendors/${formData.id}`, payload);
      setAlert({ type: 'success', message: 'Vendor updated successfully!' });
    } else {
      await axios.post("http://192.168.0.102:8000/vendors", payload);
                    ...
                    <div className="space-y-4 mt-6">
                      ...
                      <div className="space-y-2">
                        ...
                        {!gstPdfFile && formData.gstpdf && (
                          <a
                            href={`http://192.168.0.102:8000/gst/${formData.gstpdf}`}
                            target="_blank"
                  ...
                  {currentVendors.map((vendor) => (
                    ...
                    >
                      ...
                      <TableCell className="px-6 py-4">
                        {vendor.gstpdf ? (
                          <a
                            href={`http://192.168.0.102:8000/gst/${vendor.gstpdf}`}
                            target="_blank"
                            rel="noopener noreferrer"
