...
const CustomerTable: React.FC = () => {
  ...
  const handleDelete = async (id?: number) => {
    ...
    if (!confirm) return;
    try {
      await axios.delete(`http://192.168.0.102:8000/customers/${id}`);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(
          `http://192.168.0.102:8000/customers/${formData.id}`,
          form,
          {
        ...
        await axios.post("http://192.168.0.102:8000/customers", form, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
                  ...
                  {currentCustomers.map((customer) => (
                    ...
                    >
                      ...
                      <TableCell className="px-4 py-3">
                        {customer.gstpdf ? (
                          <a
                            href={`http://192.168.0.102:8000/gst/${customer.gstpdf}`}
                            target="_blank"
            ...
            <div className="space-y-4 mt-6">
              ...
              <div className="space-y-2">
                ...
                {!gstpdfFile && existingGstFileName && (
                  <a
                    href={`http://192.168.0.102:8000/gst/${existingGstFileName}`}
                    target="_blank"
                    rel="noopener noreferrer"
