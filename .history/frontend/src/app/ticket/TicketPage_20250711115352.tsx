...
export default function TicketPage() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const endpoint = `http://192.168.0.102:8000/tickets/user/${userId}`;
  ...
  const fetchCustomers = async () => {
    try {
      const res = await axios.get("http://192.168.0.102:8000/customers");
  ...
  const fetchTicketDetails = async (id: number) => {
    try {
      ...
      const res = await axios.get(`http://192.168.0.102:8000/tickets/${id}`);
      setSelectedTicket(res.data);
      const msgRes = await axios.get(`http://192.168.0.102:8000/message/${id}`);
  ...
  const fetchCurrentUser = async () => {
    try {
      if (userId) {
        const response = await axios.get(`http://192.168.0.102:8000/users/${userId}`);
  ...
  const handleCreateTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post("http://192.168.0.102:8000/tickets", ticketData);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post("http://192.168.0.102:8000/message", {
        content: newMessage,
        senderId: userId,
  ...
  const handleStatusChange = async (ticketId: number, newStatus: string) => {
    ...
    try {
      await axios.patch(`http://192.168.0.102:8000/tickets/${ticketId}`, {
        status: newStatus,
      });
  ...
  const handleDeleteTicket = async (ticketId: number) => {
    ...
    try {
      await axios.delete(`http://192.168.0.102:8000/tickets/${ticketId}`, {
