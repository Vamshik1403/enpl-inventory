...
const SubCategoryTable: React.FC = () => {
  ...
  const fetchSubCategories = async () => {
    try {
      const response = await axios.get("http://http://192.168.29.167:8000/subcategory");
  ...
  const handleDelete = async (subCategoryId: number) => {
    if (window.confirm("Are you sure you want to delete this subcategory?")) {
      try {
        await axios.delete(
          `http://http://192.168.29.167:8000/subcategory/${subCategoryId}`
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedSubCategory) {
        ...
        await axios.put(
          `http://http://192.168.29.167:8000/subcategory/${selectedSubCategory.id}`,
          {
            categoryId,
            subCategoryName,
        ...
        await axios.post("http://http://192.168.29.167:8000/subcategory", {
          categoryId,
          subCategoryName,
          subCategoryId: newSubCategoryId,
        });
