...
const paginatedInventory = sortedInventory.slice(
  ...
  const fetchData = async () => {
    try {
      const [inventoryRes, paymentsRes] = await Promise.all([
        axios.get("http://192.168.0.102:8000/inventory"),
        axios.get("http://192.168.0.102:8000/vendor-payment"),
      ]);
  ...
  const fetchProducts = async () => {
    const res = await axios.get("http://192.168.0.102:8000/products");
    setProducts(res.data);
  };

  const fetchVendors = async () => {
    const res = await axios.get("http://192.168.0.102:8000/vendors");
    setVendors(res.data);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.id) {
        await axios.put(
          `http://192.168.0.102:8000/inventory/${formData.id}`,
          payload
        );
        setAlert({ type: "success", message: "Inventory updated!" });
      } else {
        await axios.post("http://192.168.0.102:8000/inventory", payload);
  ...
  const handleDelete = async (id?: number) => {
    ...
    await axios.delete(`http://192.168.0.102:8000/inventory/${id}`);
    fetchData();
  };
