...
const DepartmentTable: React.FC = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post("http://http://192.168.29.167:8000/departments", formData);
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `http://http://192.168.29.167:8000/departments/${formData.id}`,
        formData
      );
  ...
  const handleDelete = useCallback(
    async (id: number) => {
      if (window.confirm("Are you sure you want to delete this department?")) {
        try {
          await axios.delete(`http://http://192.168.29.167:8000/departments/${id}`);
